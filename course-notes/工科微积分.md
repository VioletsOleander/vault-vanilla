...
若函数在 $x_0$ 处的某邻域内有定义，当 $x$ 在 $x_0$ 处的增量趋于 0 时，函数的相应增量也趋于 0，即 $$\lim_{\Delta x \to 0}\Delta y = 0$$
...
从几何上看，反函数和直接函数的图形是同一条曲线，设曲线上处处有切线，过曲线上一点 $M(x,y)$ 作切线，该切线与 $x$ 轴，$y$ 轴的夹角分别是 $\alpha$，$\beta$，显然 

$$\tan \alpha = \cot \beta = \frac 1 {\tan \beta}$$
...
证明：
因为 $f(x)$ 在 $[a,b]$ 上连续，故可积，设 $f(x)$ 在 $[a,b]$ 上的最大值和最小值分别是 $M,m$，则 
...
由连续函数的介值定理，至少存在一点 $\xi\in[a,b]$ 使得 
