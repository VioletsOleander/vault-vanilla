...
A reward function $R(s, a, s')$ (sometimes simplified as $R(s, a)$)
...
# 3 RL and Function Approximation
...
# 4 Deep Q Learning & Policy Gradient
## Policy Gradient
...
### Twin Delayed DDPG
DDPG suffers similar problems with DQN: overestimation
The critics in DDPG might by unstable. 
The critics weirdly prefers some action but not their neighbor actions, which leads to strange landscape.

To address overestimation: Clipped Double Q Learning

$$
y(r, s', d) = r + \gamma(1-d)\min_{i=1, 2} Q_{\phi_i,targ}(s',a_{TD3}(s))
$$

To address unstable critics: Delayed Policy Updates
lower the frequency of actor updates.

For every $N$ updates in critics, we update policy once.

To address strange landscape: Target Policy Smoothing
...
$$
\text{maximize}_\theta \hat {\mathbb E}_t\left[\frac {\pi_\theta(a_t\mid s_t)}{\pi_{\theta_{old}}(a_t\mid s_t)}\hat A_t\right] -\beta\hat{\mathbb E}_t\left[\text{KL}(\pi_{\theta_{old}}(\cdot \mid s_{t}),\pi_\theta(\cdot\mid s_t))\right]
$$

PPO is actually an simplified version of trust-region policy optimization (TRPO).
...
Let $r_t(\theta) = \frac {\pi_\theta(a_t\mid s_t)}{\pi_{\theta_{old}}(a_t\mid s_t)}$. In TRPO, since we only have a soft constraints on the KL divergence, fluctuation happens when the ratio is too large.

PPO turns the objective with adaptive KL penalty into a clipped objective
...
Why advantage?
In the policy gradient methods, we are actually using the critic evaluating the advantage and improve the policy based on it

$$
L^{PG}(\theta) = \hat {\mathbb E}_t[\log \pi_\theta(a_t\mid s_t)\hat A_t]
$$

Policy gradient is a "soft" version of policy iteration, both of which are based on:

$$
J(\theta') - J(\theta) = \mathbb E_{\tau\sim p_{\theta'}(\tau)}[\sum_{t=0}^\infty \gamma^t A_{\pi_{\theta}}(s_t, a_t)]
$$

Policy iteration totally trust the advantage, and is greedy with respect to it. Policy gradient give less trust, and the update scale is controlled by the learning rate.
...
\begin{align}
J(\theta') - J(\theta) &= \mathbb E_{\tau\sim p_{\theta'}(\tau)}[\sum_{t=0}^\infty \gamma^t A_{\pi_{\theta}}(s_t, a_t)]\\
&=\sum_{t=0}^\infty \mathbb E_{s_t\sim p_{\theta'}(s_t)}[\mathbb E_{a_t\sim \pi_{\theta'}(a_t\mid s_t)}[\gamma^t A_{\pi_{\theta}}(s_t, a_t)]]\\
&=\sum_{t=0}^\infty \mathbb E_{s_t\sim p_{\theta'}(s_t)}[\mathbb E_{a_t\sim \pi_{\theta}(a_t\mid s_t)}[\frac {\pi_{\theta'}(a_t\mid s_t)}{\pi_\theta(a_t\mid s_t)}\gamma^t A_{\pi_{\theta}}(s_t, a_t)]]\\
\end{align}
$$

The state distribution $p_{\theta'}(s_t)$ will be replaced by $p_\theta(s_t)$, and the TRPO paper gives a relative bound about this approximation.
The intuition is: $p_\theta(s_t)$ is close to $p_{\theta'}(s_t)$ when $\pi_\theta$ is close to $\pi_{\theta'}$, so the approximation can be justified.

Thinking about: $p_\theta(s_t)$ is close to $p_{\theta'}(s_t)$ when $\pi_\theta$ is close to $\pi_{\theta'}$
Simple case: assume $\pi_\theta$ is a deterministic policy $a_t = \pi_\theta(s_t)$, then we say $\pi_{\theta'}$ is close to $\pi_\theta$ if $\pi_{\theta'}(a_t\ne \pi_\theta(s_t)\mid s_t)\le \epsilon$ 

If $\pi_{\theta'}$ is close to $\pi_\theta$, then $p_{\theta'}(s_t) = (1-\epsilon)^t p_\theta(s_t) + (1-(1-\epsilon)^t) p_{mistake}(s_t)$, and:
...
we used the inequality $(1-\epsilon)^t \ge 1 - \epsilon t$ for $\epsilon \in [0, 1]$ in the first inequality to derive the second inequality.

General case: assume $\pi_\theta$ is an arbitrary distribution, then we say $\pi_{\theta'}$ is close to $\pi_\theta$ if $|\pi_{\theta'}(a_t\mid s_t) - \pi_\theta(a_t\mid s_t)| \le \epsilon$ for all $s_t$, 
...
If the distance between two distribution are bounded: $|p_{\theta'}(s_t) - p_\theta(s_t)|\le 2\epsilon t$, we will have

$$
\begin{align}
\mathbb E_{p_{\theta'}(s_t)}[f(s_t)] &= \sum_{s_t}p_{\theta'}(s_t)f(s_t)\\
&\ge \sum_{s_t}p_\theta(s_t)f(s_t)- |p_\theta(s_t)- p_{\theta'}(s_t)|\max_{s_t}f(s_t)\\
&\ge \mathbb E_{p_\theta(s_t)}[f(s_t)]- 2\epsilon t\max_{s_t} f(s_t)
\end{align}
$$
...
$$
\begin{align}
&\sum_t\mathbb E_{s_t\sim p_{\theta'}(s_t)}\left[\mathbb E_{a_t\sim \pi_\theta(a_t\mid s_t)}\left[\frac {\pi_{\theta'}(a_t\mid s_t)}{\pi_\theta(a_t\mid s_t)}\gamma^t A_{\pi_\theta}(s_t, a_t)\right]\right]\\
\ge&\sum_t\mathbb E_{s_t\sim p_{\theta}(s_t)}\left[\mathbb E_{a_t\sim \pi_\theta(a_t\mid s_t)}\left[\frac {\pi_{\theta'}(a_t\mid s_t)}{\pi_\theta(a_t\mid s_t)}\gamma^t A_{\pi_\theta}(s_t, a_t)\right]\right]-\sum_t2\epsilon C\\
\end{align}
$$

We get a lower bound of the policy improvement.

Why KL divergence
Now we consider how to define $\epsilon$. We can use this bound

$$
|\pi_{\theta'}(a_t\mid s_t) - \pi_{\theta}(a_t\mid s_t)|\le \sqrt {\frac 1 2D_{\text{KL}}(\pi_{\theta'}(a_t\mid s_t)||\pi_\theta(a_t\mid s_t))}
$$

Therefore, we turn to use KL divergence to measure the small distance $\epsilon$ , and for small enough KL divergence, the $\epsilon$ will be small enough, and the policy objective ($J(\theta') - J(\theta)$) is guaranteed to improve

That is, we solve this constraint optimization problem:

$$
\arg\max_{\theta'}\sum_t\mathbb E_{s_t\sim p_{\theta}(s_t)}\left[\mathbb E_{a_t\sim \pi_\theta(a_t\mid s_t)}\left[\frac {\pi_{\theta'}(a_t\mid s_t)}{\pi_\theta(a_t\mid s_t)}\gamma^t A_{\pi_\theta}(s_t, a_t)\right]\right], s.t. D_{KL}(\pi_{\theta'}||\pi_\theta)\le \epsilon\\
$$

Then $J(\theta')- J(\theta)$ can be guaranteed by the previous described bound.

# 7 Model-based Reinforcement Learning
# 8 Learning from Demonstrations
# 9 Diffusion Models for RL/IL
