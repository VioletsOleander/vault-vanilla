...
我们需要证明 $\mathcal Q(\mathbf x)$ 存在以下唯一展开形式：
...
对于 $N$ 的任意子集 $J \subseteq [n]$，有两个定义：
(1) 定义 $\mathbf x_J$ 为 $\mathbf x |_J = (x_i : x\in J)$，也就是函数 $\mathbf x$ 在集合 $J$ 的限制

(2) 定义 $\tilde {\mathbf x}_J$ 为 $[n]\to \mathbb R$ 的函数，满足：
...
对于 $N$ 的任意子集 $K \subseteq [n]$，定义：

$$
H_K(\mathbf x_K)  = \sum_{J \subseteq K}(-1)^{|K|-|J|}\mathcal Q(\tilde {\mathbf x}_J)
$$

对于 $H_K(\mathbf x_K)$，有两个结论：
(1) 对于所有的 $K\subseteq [n]$ 的 $H_K (\mathbf x_K)$ 求和的结果是 $\mathcal Q (\mathbf x)$
...
(2) 如果对于某个 $j\in K$，有 $x_j = 0$，则 $H_K (\mathbf x _K) = 0$

证明：
对于每个满足 $j\in J$ 的子集 $J\subseteq K$，存在集合 $J' = J\backslash \{j\}$ 不包含 $j$，同时显然有 $J' \subseteq K$

因为 $x_j = 0$，故 $\tilde {\mathbf x}_J$ 和 $\tilde {\mathbf x}_{J'}$ 显然相等，即 $\tilde {\mathbf x}_J = \tilde {\mathbf x}_{J'}$，故 $\mathcal Q (\tilde {\mathbf x}_J) = \mathcal Q (\tilde {\mathbf x}_{J'})$
又因为 $J' = J\backslash \{j\}$，故 $|J'| = |J| - 1$，因此 $(-1)^{|K| - |J|}$ 和 $(-1)^{|K|-|J'|}$ 一定符号相反

因此 $J$ 和 $J'$ 在和式中的共同贡献满足 $(-1)^{|K|-|J|}\mathcal Q(\tilde {\mathbf x}_J) + (-1)^{|K|-|J'|}\mathcal Q(\tilde {\mathbf x}_{J'}) = 0$

根据该思路，我们将 $H_K (\mathbf x_K)$ 重写为：

$$
\begin{align}
H_K(\mathbf x_K)  &= \sum_{J \subseteq K}(-1)^{|K|-|J|}\mathcal Q(\tilde {\mathbf x}_J)\\
&=\sum_{J \subseteq K,j\in J}(-1)^{|K|-|J|}\mathcal Q(\tilde {\mathbf x}_J) + \sum_{J \subseteq K,j\not\in J}(-1)^{|K|-|J|}\mathcal Q(\tilde {\mathbf x}_J)\\
&=\sum_{J \subseteq K,j\in J}(-1)^{|K|-|J|}\mathcal Q(\tilde {\mathbf x}_J) + \sum_{J' \subseteq K\backslash\{j\}}(-1)^{|K|-|J'|}\mathcal Q(\tilde {\mathbf x}_{J'})\\
&=\sum_{J' \subseteq K\backslash\{j\}}(-1)^{|K|-(|J'| + 1)}\mathcal Q(\tilde {\mathbf x}_{J'}) + \sum_{J' \subseteq K\backslash\{j\}}(-1)^{|K|-|J'|}\mathcal Q(\tilde {\mathbf x}_{J'})\\
&=0
\end{align}
$$
...
定义 $p_{K}(\mathbf x)$ 为 $p_K(\mathbf x) = \prod_{j\in K}x_j$ ，根据该结论，我们容易推出：当 $p_{K}(\mathbf x) = 0$ 时，$H_K (\mathbf x) = 0$

我们将 $G_K (\mathbf x)$ 定义为

$$G_K (\mathbf x) =\begin{cases}
\frac {H_K (\mathbf x_K)}{p_K (\mathbf x)} & p_K(\mathbf x) \ne 0\\
0 & p_K(\mathbf x) = 0
\end{cases}$$

显然，对于所有的 $\mathbf x$ 和所有的 $K$，满足 $H_K (\mathbf x_K) = p_K (\mathbf x) G_K (\mathbf x_K)$

将式 (2) 中的 $H_K (\mathbf x_K)$ 替换为 $p_K (\mathbf x) G_K (\mathbf x_K)$，我们得到

$$
\mathcal Q(\mathbf x) = \sum_{K\subseteq [n]} p_K(\mathbf x)G_K(\mathbf x_K)\tag{3}
$$

显然，式 (3) 就是式 (1) 的另一种写法

$G_K (\mathbf x_K)$ 可以视为函数 $\mathcal Q$ 关于 $\mathbf x_K$ 的偏分差 (partial divided difference)，其阶数为 $|K|$，也就是说，对于给定的 $K\subseteq [n]$，$G_K (\mathbf x_K)$ 定义为

$$
\begin{align}
G_K(\mathbf x_K)&= \frac {\Delta_K \mathcal Q(\mathbf x)}{\prod_{i\in K}x_i}\\
&=\frac {H_K(\mathbf x_K)}{\prod_{i\in K}x_i}
\end{align}
$$

其中 $\Delta_K \mathcal Q (\mathbf x)$ 是 $\mathcal Q$ 在 $\mathbf x$ 上关于 $K$ 的差分

例如，$G_{\{1\}}(\mathbf x_{\{1\}})$ 写为

$$
G_{\{1\}}(\mathbf x_{\{1\}}) = \frac {\mathcal Q(x_1,0,\dots, 0) - \mathcal Q(0, \dots, 0)}{x_1}
$$

其中 $x_1 \ne 0$

又例如，$G_{\{1, 2\}}(\mathbf x_{\{1, 2\}})$ 写为

$$
G_{\{1, 2\}}(\mathbf x_{\{1, 2\}}) = \frac {\mathcal Q(x_1, x_2, \dots, 0) - \mathcal Q(x_1, 0, \dots, 0) - \mathcal Q(0,x_2, \dots, 0) + \mathcal Q(0,\dots, 0)}{x_1x_2}
$$

其中 $x_1, x_2\ne 0$

在这种语境下，式 (1) 以及式 (3) 可以与对连续函数的麦克劳林展开进行类比：

对于一个光滑函数 $f(x)$，其麦克劳林展开为：

$$
f(x) = f(0) + \frac {f'(0)}{1!} x + \frac {f''(0)}{2!}x^2 + \cdots + \frac {f^{(n)}(0)}{n!}x^n+\cdots
$$

对于一个 $n$ 变量函数 $\mathcal Q (\mathbf x)$，它展开为：
...
其中 $\mathcal Q (\mathbf 0) = 0$

展开式 (1) 和 (3) 是唯一的，也就是对于所有满足 $p_K (\mathbf x) \ne 0$ 的子集 $K$ 和赋值 $\mathbf x$，都有唯一决定的 $G_K (\mathbf x_K)$
