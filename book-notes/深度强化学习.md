...
16. 用策略网络 $\pi_{\mathrm{now}}$ 控制智能体与环境交互，完成一个回合，得到轨迹： 

$$
s_{1},a_{1},r_{1},\ \ s_{2},a_{2},r_{2},\ \ \cdot\cdot\cdot,\ \ s_{n},a_{n},r_{n}.
$$ 
17. 对于所有的 $t=1,\cdot\cdot\cdot\,,n-m$ ，计算 

$$
\widehat{q}_{t}\;=\;q\big(s_{t},a_{t};\pmb{w}_{\mathrm{now}}\big).
$$ 
18. 对于所有的 $t=1,\cdot\cdot\cdot\,,n-m$ ，计算多步 TD 目标和 TD 误差： 

$$
\widehat{y}_{t}\;=\;\sum_{i=0}^{m-1}\gamma^{i}r_{t+i}\,+\,\gamma^{m}\widehat{q}_{t+m},\qquad\delta_{t}\;=\;\widehat{q}_{t}-\widehat{y}_{t}.
$$ 
19. 对于所有的 $t=1,\cdot\cdot\cdot\,,n-m$ ，对价值网络 $q$ 做反向传播，计算 $q$ 关于 $\mathbfit{w}$ 的梯度： 
...
$$ 
21. 用某种算法更新策略函数 $\pi$ 。该算法与 SARSA 算法无关。 
...
58. 用策略网络 $\scriptstyle\theta_{\mathrm{now}}$ 控制智能体从头开始玩一局游戏，得到一条轨迹(trajectory)： 
$$
...
$$ 
60. 用 $\{(s_{t},a_{t})\}_{t=1}^{n}$ 作为数据，做反向传播计算： 
    
$$
...
62. 观测到当前状态 $s_{t}$ ，根据策略网络做决策： $a_{t}\sim\pi(\,\cdot\,|\,s_{t};\,\theta_{\mathrm{now}})$ ，并让智能体执行动 
作 $a_{t}$ 。 
63. 从环境中观测到奖励 $r_{t}$ 和新的状态 $s_{t+1}$ 。 
64. 根据策略网络做决策： $\tilde{a}_{t+1}\sim\pi\big(\cdot|s_{t+1};\theta_{\mathrm{now}}\big)$ ，但不让智能体执行动作 $\tilde{a}_{t+1}$ 。 
65. 让价值网络打分： 
      
$$
...
69. 观测到当前状态 $s_{t}$ ，根据策略网络做决策： $a_{t}\sim\pi(\,\cdot\,|\,s_{t};\,\theta_{\mathrm{now}})$ ，并让智能体执行动作 $a_{t}$ 。 
70. 从环境中观测到奖励 $r_{t}$ 和新的状态 $s_{t+1}$ 。 
71. 根据策略网络做决策： $\tilde{a}_{t+1}\sim\pi\big(\cdot\,\big|\,s_{t+1};\,\theta_{\mathrm{now}}\big)$ ，但是不让智能体执行动作 $\tilde{a}_{t+1}$ 。 
72. 让价值网络给 $(s_{t},a_{t})$ 打分： 
   
$$
\widehat{q}_{t}\;=\;q\big(s_{t},a_{t};\,\pmb{w}_{\mathrm{now}}\big).
$$
 b 
73. 让目标网络给 $(s_{t+1},\tilde{a}_{t+1})$ 打分： 
   
$$
...
$$
 b 
77. 设 $\tau\in(0,1)$ 是需要手动调整的超参数。做加权平均更新目标网络的参数： 
   
$$
...
# $\iff$ 第7 章习题k 
78. 把策略网络记作 $\pi(a|s103. 把策略网络记作 $\pi(a|s;\pmb\theta)$ 。请计算： $\begin{array}{r}{\sum_{a\in\mathcal{A}}\left|\pi(a|s;\pmb\theta)\right|\,=\,\underline }\end{array}$ 。∈A }\end{array}$ 。∈A ax 做激活函数？ 
79. 状态价值函数 $V_{\pi}(s_{t})$ 依赖于未来的状态 $s_{t+1},s_{t+1},\cdot\cdot\cdot$ 。 
A. 上述说法正确。B. 上述说法错误。 
80. 设 $\ln x$ 为 $x$ 的自然对数。它的导数是 ${\frac{d\ln x}{d x}}={\frac{}{-\ \ \ \ \ \ \ }}$ 。 
A. $x^{2}$ B. $x$ C. $\textstyle{\frac{1}{x}}$ D. ln $x$ E. $e^{x}$ 
81. REINFORCE 的原理是对策略梯度 $\frac{\partial\,J(\theta)}{\partial\,\theta}$ 做蒙特卡洛近似。请你总结一下REIN-FORCE 做了哪些近似。 
82. 在actor-critic 的训练中使用目标网络，目的在于。 
...
83. 用策略网络 $\scriptstyle\theta_{\mathrm{now}}$ 控制智能体从头开始玩一局游戏，得到一条轨迹（trajectory）： 
$$
s_{1},a_{1},r_{1},\quad s_{2},a_{2},r_{2},\quad\cdots\cdot,\quad s_{n},a_{n},r_{n}.
$$ 
84. 计算所有的回报： 
...
$$ 
86. 计 $\delta_{t}=\widehat{v}_{t}-u_{t}$ ， $\forall\,t=1,\cdot\cdot\cdot\,,n\,_{\odot}$ ∀ · · ·。 
87. 用 $\{s_{t}\}_{t=1}^{n}$ {} 作为价值网络输入，做反向传播计算： 
   
$$
...
$$ 
89. 用 $\{(s_{t},a_{t})\}_{t=1}^{n}$ 作为数据，做反向传播计算： 
    
$$
\nabla_{\theta}\ln\pi\big(a_{t}\,\big|\,s_{t};\,\theta_{\mathrm{now}}\big),\qquad\forall\,t=1,\cdot\cdot\cdot\,,n.
$$ 
...
97. 观测到当前状态 $s_{t}$ ，根据策略网络做决策： $a_{t}\sim\pi(\,\cdot\,|\,s_{t};\,\theta_{\mathrm{now}})$ ，并让智能体执行动作 $a_{t}$ 。 
98. 从环境中观测到奖励 $r_{t}$ 和新的状态 $s_{t+1}$ 。 
99. 让价值网络给 $s_{t}$ 打分： 
   
$$
...
\pmb{\theta}_{\mathrm{new}}\ \leftarrow\ \pmb{\theta}_{\mathrm{now}}\ -\ \beta\cdot\delta_{t}\cdot\nabla_{\pmb{\theta}}\ln\pi\big(a_{t}\,\big|\,s_{t};\,\pmb{\theta}_{\mathrm{now}}\big).
$$ 
104. 设 $\tau\in(0,1)$ 是需要手动调的超参数。做加权平均更新目标网络的参数： 
   
$$
\pmb{w}_{\mathrm{new}}^{-}\ \leftarrow\ \tau\cdot\pmb{w}_{\mathrm{new}}\,+\,\left(1-\tau\right)\cdot\pmb{w}_{\mathrm{new}}^{-}.
$$ 
...
# $\iff$ 第8 章习题k 
105. 是否可以用最优动作价值函数 $Q_{\star}(s,a)$ 作为基线？ 
A. 可以，因为 $Q_{\star}$ 不依赖于动作。B. 不行，因为 $Q_{\star}$ 依赖于动作。 
106. 带基线的REINFORCE 算法属于Actor-Critic。 
...
107. 做近似——构造函数 $\tilde{L}$ 近似目标函数 $J(\pmb\theta)$ ： 
...
114. 让目标策略网络 $\widehat{\pmb{a}}_{j+1}^{-}=\pmb{\mu}(s_{j+1};\pmb{\theta}_{\mathrm{now}}^{-})+\pmb{\xi}$ 。其中向量 $\xi$ 的每个元素都独立从截断正态分布 $\mathcal{C N}(0,\,\sigma^{2},-c,c)$ CN − 中抽取。 
115. 让两个目标价值网络做预测： 
      
$$
...
$$ 
120. 每隔 $k$ 轮更新一次策略网络和三个目标网络：让策略网络做预测： $\widehat{\mathbf{a}}_{j}=\pmb{\mu}(s_{j};\pmb{\theta})$ 。然后更新策略网络： 
      
$$
...
121. 观测到当前状态 $s$ ，预测均 ${\widehat{\mu}}=\mu(s)$ 标准差 ${\widehat{\sigma}}=\sigma(s)$ 。 
122. 从正态分布中做随机抽样： $\boldsymbol{a}\sim\mathcal{N}(\widehat{\mu},\widehat{\sigma}^{2})$  ；智能体执行动作 $a$ 。 
...
123. 当前状态 $s$ ，计算均值 $\widehat{\mu}=\mu(s;\pmb\theta)$ ，方差对数 ${\widehat{\rho}}=\rho(s;\pmb\theta)$ ，以及方差 $\widehat{\sigma}^{2}=$ $\exp({\widehat{\rho}})\,.$ 。 
124. 从正态分布中做随机抽样：b $a\sim\mathcal{N}(\widehat{\mu},\widehat{\sigma}^{2})$ ；智能体执行动作 $a$ 。 
...
125. 搭建均值网络 $\pmb{\mu}(s;\pmb{\theta})$ 、方差对数网络 $\rho(s;\pmb\theta)$ 、辅助网络 $f(s,\mathbf{}\mathbf{}\mathbf{}\mathbf{};\pmb{\theta})$ 。2. 让智能体与环境交互，记录每一步的状态、动作、奖励，并对参数 $\pmb{\theta}$ 做更新： 
...
126. 用 $\pmb{\mu}(s;\pmb{\theta}_{\mathrm{now}})$ 和 $\pmb{\rho}(s;\pmb{\theta}_{\mathrm{now}})$ 控制智能体与环境交互，完成一局游戏，得到一条轨迹： 
$$
...
130. 实际观测到当前状态 $s_{t}$ ，用控制器算出均值 $\pmb{\mu}(s_{t};\pmb{\theta}_{\mathrm{now}})$ 和方差对数 $\pmb{\rho}(s_{t};\pmb{\theta}_{\mathrm{now}})$ ，然后随机抽样得到动作 $\mathbf{\delta}a_{t}$ 。智能体执行动作 $\mathbf{a}_{t}$ ，观测到奖励 $r_{t}$ 与新的状态 $s_{t+1}$ 。 
131. 计算均值 $\pmb{\mu}(s_{t+1};\pmb{\theta}_{\mathrm{now}})$ 和方差对数 $\pmb{\rho}(s_{t+1};\pmb{\theta}_{\mathrm{now}})$ ，然后随机抽样得到动作 $\tilde{\mathbf{a}}_{t+1}$ 。这个动作只是假想动作，智能体不予执行。 
132. 用价值网络计算出： 
   
$$
...
139. 从序号 $\{1,\cdot\cdot\cdot\ ,n\}$ 中做均匀随机抽样，把抽到的序号记作 $j$ 。 
140. 设当前策略网络参数为 $\theta_{\mathrm{now}}$ 。把 $s_{j}\backsim\mathbf{\nabla}a_{j}$ 作为输入，做反向传播计算梯度，然后用梯度更新 $\pmb{\theta}$ ： 
   
$$
...
141. 从序号 $\{1,\cdot\cdot\cdot\ ,n\}$ 中做均匀随机抽样，把抽到的序号记作 $j$ 。 
142. 设当前策略网络的参数是 $\scriptstyle\theta_{\mathrm{now}}$ 。把 $s_{j},\ \bar{\mathbf{a}}_{j}$ 作为输入，做反向传播计算梯度，然后用梯度更新 $\pmb{\theta}$ ： 
$$
...
143. 更新判别器 $D(x;\phi)$ 的参数：(a). 计算平均梯度： 
$$
...
144. 从训练数据集中均匀抽样一条轨迹，记作 
$$
\begin{array}{r l r}{\tau^{\mathrm{real}}}&{=}&{\left[s_{1}^{\mathrm{real}},\,a_{1}^{\mathrm{real}},\,\,\cdot\cdot\cdot\,,\,\,s_{m}^{\mathrm{real}},a_{m}^{\mathrm{real}}\right].}\end{array}
$$ 
145. 用策略网络 $\pi(a|s;\theta_{\mathrm{now}})$ 控制智能体与环境交互，得到一条轨迹，记作 
$$
...
$$ 
146. 用判别器评价策略网络的决策是否真实： 
$$
u_{t}\ =\ \ln D\Big(s_{t}^{\mathrm{fake}},\,a_{t}^{\mathrm{fake}};\ \phi_{\mathrm{now}}\Big),\qquad\forall\,t=1,\cdot\cdot\cdot\,,n.
$$ 
147. 把 $\tau^{\mathrm{fake}}$ 和 $u_{1},\cdot\cdot\cdot\ ,u_{n}$ 作为输入，用公式(12.2) 更新生成器（即策略网络）的参数，得到 $\pmb{\theta}_{\mathrm{new}}$ 。 
148. 把 $\tau^{\mathrm{real}}$ 和 $\tau^{\mathrm{fake}}$ 作为输入，用公式(12.3) 更新判别器参数，得到 $\phi_{\mathrm{new}}$ 。 
# $\backsimeq$ 第12 章知识点N 
...
149. 广播（broadcast）：服务器将当前的模型参数 ${\pmb w}_{\mathrm{n ow}}$ 广播到 $m$ 个worker 节点。这样一来，所有节点都知道 ${\pmb w}_{\mathrm{nlow}}$ 。 
150. 映射（map）：这一步让 $m$ 个worker 节点做并行计算，用本地数据计算梯度。需要 
...
151. 规约（reduce）：在做完映射之后，向量 $\boldsymbol{z}_{1},\cdots,\boldsymbol{z}_{n}\in\mathbb{R}^{d}$ 分布式存储在 $m$ 个worker节点上，每个节点有一个子集。不难看出，目标函数 $\begin{array}{r}{L(\pmb{w})=\frac{1}{2n}\sum_{j=1}^{n}(\pmb{x}_{j}^{T}\pmb{w}-y_{j})^{2}}\end{array}$ P在 ${\pmb w}_{\mathrm{now}}$ 处的梯度等于： 
$$
...
152. 更新参数：最后，服务器在本地做梯度下降，更新模型参数： 
   
$$
...
153. 钟表时间（wall-clock time），也叫elapsed real time，意思是程序实际运行的时间。可以这样理解钟表时间：在程序开始运行的时候，记录下墙上钟表的时刻；在程序结束的时候，再记录钟表的时刻；两者之差就是钟表时间。 
154. 处理器时间(CPU time 或GPU time) 是所有处理器运行时间的总和。比如使用4 块CPU 做并行计算，程序运行的钟表时间是1 分钟，期间CPU 没有空闲，那么系统的CPU 时间等于4 分钟。 
...
155. 向服务器发出请求，索要最新的模型参数。把接收到的参数记作 ${\pmb w}_{\mathrm{now}}$ 。 
156. 利用本地的数据 $\left\{(\pmb{x}_{j},y_{j})\right\}_{j\in\mathcal{Z}_{k}}$ 	 和参数 ${\pmb w}_{\mathrm{nlow}}$ 计算本地的梯度：∈I 
$$
\tilde{\pmb g}^{k}\ =\ \frac{1}{|\mathcal{Z}_{k}|}\sum_{j\in\mathcal{Z}_{k}}\big(\pmb x_{j}^{T}\pmb w_{\mathrm{now}}-y_{j}\big)\pmb x_{j}.
$$ 
157. 把计算出的梯度 $\tilde{g}^{k}$ 发送给服务器。 
...
158. 不难证明，同步并行梯度下降更新参数的方式为： 
   
$$
\pmb{w}_{\mathrm{new}}\ \leftarrow\ \pmb{w}_{\mathrm{new}}\ -\ \eta\cdot\nabla_{\pmb{w}}L\big(\pmb{w}_{\mathrm{new}}\big),
$$ 
即标准的梯度下降。在同一时刻，所有worker 节点上的参数是相同的，都是 ${\pmb w}_{\mathrm{nlow}}$ 。所有worker 节点都基于相同的 ${\pmb w}_{\mathrm{now}}$ 计算梯度。 
159. 对于异步并行梯度下降，在同一时刻，不同worker 节点上的参数 $\mathbfit{w}$ 通常是不同的。比如两个worker 分别在 $t_{1}$ 和 $t_{2}$ 时刻向服务器索要参数。在两个时刻之间，服务器可能已经对参数做了多次更新，导致在 $t_{1}$ 和 $t_{2}$ 时刻取回的参数不同。两个worker节点会基于不同的参数计算梯度。 
...
160. 向服务器发出请求，索要最新的DQN 参数。把接收到的参数记作 $\pmb{w}_{\mathrm{new}}$ 。 
161. 更新本地的目标网络： 
   
$$
\pmb{w}_{\mathrm{new}}^{-}\ \leftarrow\ \tau\cdot\pmb{w}_{\mathrm{new}}\,+\,\left(1-\tau\right)\cdot\pmb{w}_{\mathrm{new}}^{-}.
...
\tilde{\pmb{g}}^{k}\ =\ \nabla_{\pmb{w}}\,L\big(\pmb{w}_{\mathrm{new}}\big).
$$ 
163. 把计算出的梯度 $\tilde{\boldsymbol{g}}^{k}$ 发送给服务器。 
...
164. 向服务器发出请求，索要最新的参数。把接收到的参数记作 $\pmb{\theta}_{\mathrm{new}}$ 、 $\pmb{w}_{\mathrm{new}}$ 。 
165. 更新本地的目标网络： 
   
$$
\pmb{w}_{\mathrm{new}}^{-}\ \leftarrow\ \tau\cdot\pmb{w}_{\mathrm{new}}\,+\,\left(1-\tau\right)\cdot\pmb{w}_{\mathrm{new}}^{-}.
$$ 
166. 重复下面的步骤 $b$ 次（ $\dot{\mathbf{\eta}}^{b}$ 是用户设置的超参数），或是从头到尾完成一回合游戏。让智能体与环境交互，计算策略梯度，并累积策略梯度。全零初始化： $\tilde{g}_{\theta}^{k}\gets\mathbf{0}$ ←、 $\tilde{g}_{w}^{k}\gets\mathbf{0}$ ←，用它们累积梯度。 
...
$$ 
167. 把累积的梯度 $\tilde{\pmb g}_{\theta}^{k}$ 和 $\tilde{\pmb{g}}_{w}^{k}$ 发送给服务器。 
...
# $\iff$ 第13 章习题k 
168. 如果在1 个节点（compute node）上运行某算法，需要100 秒才能收敛。如果在8个节点上运行该算法，需要20 秒收敛。加速比（speedup ratio）等于。 
169. 使用更多的节点做并行计算，则通信所需的钟表时间会。 
A. 升高B. 降低 C. 不变 
170. 单机的梯度下降算法与同步并行梯度下降算法在数学上是等价的。 
A. 这种说法正确。B. 这种说法错误。 
171. 同步并行梯度下降算法与异步并行梯度下降算法在数学上是等价的。 
A. 这种说法正确。B. 这种说法错误。 
# u 第13 章相关文献U 
...
223. 让MCTS 自我博弈，完成一局游戏，收集到 $n$ 个三元组 $(s_{1},p_{1},u_{1}),\cdot\cdot\cdot,(s_{n},p_{n},u_{n})\,\mathrm{.}$ 。2. 按照公式(18.2) 做一次梯度下降，更新策略网络参数 $\pmb{\theta}$ 。3. 按照公式(18.3) 做一次梯度下降，更新价值网络参数 $\mathbfit{w}$ 。 
# u 第18 章相关文献U 
...
# $\iff$ 第18 章习题k 
224. AlphaGo 用MCTS 来判断动作的好坏。通过计算，MCTS 发现动作 $a$ 的分数 $N(a)$ 很高，这说明。 
A. 动作 $a$ 好B. 动作 $a$ 不好 
225. AlphaGo 中的价值网络 $v(s;w)$ 是对的近似。 
