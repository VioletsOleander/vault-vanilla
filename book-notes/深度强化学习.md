...
$$ 
1. 设 $\tau\in (0,1)$ 是需要手动调的超参数。做加权平均更新目标网络的参数： 
...
$$ 
1. 求值——即计算 $(s_{j+1}, a^{\star})$ 的价值，从而算出 TD 目标： 
...
$$ 
1. 设 $\tau\in (0,1)$ 是需要手动调整的超参数。做加权平均更新目标网络的参数： 
...
$$ 
1. 用标准正态分布生成 $\pmb \xi^{\prime}$ 。用目标网络计算价值： 
...
1. 设 $\alpha_{\mu}$ 和 $\alpha_{\sigma}$ 为学习率。做梯度下降更新噪声 DQN 的参数： 
