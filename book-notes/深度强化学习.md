...
9. 把表格 ${\widetilde Q}_{\mathrm{now}}$ e 中第 $({s}_{j},{a}_{j})$ 位置上的元素记作： 
   
$$
\begin{array}{r}{\widehat{q}_{j}\ =\ \widetilde{Q}_{\mathrm{now}}\bigl(s_{j},a_{j}\bigr).}\end{array}
$$
 b e 
10. 查看表格 ${\widetilde Q}_{\mathrm{now}}$ 的第 $s_{j+1}$ 行，把该行的最大值记作： 
$$
...
14. 设 $\mathcal{A}=\left\{\begin{array}{r l r l}\end{array}\right.$ 上，下，左，右 $\}$ 为动作空间， $s_{t}$ 为当前状态， $Q_{\star}$ 为最优动作价值函数。策略函数输出： 
$$
...
16. 设状态空间、动作空间的大小分别是 $|{\mathcal{S}}|=3$ 、 $|\mathcal{A}|=4$ 。如图4.8 所示，最优动作价值函数 $Q_{\star}$ 可以表示为表格形式。设 $s$ 为第3 种状态。那么 $\begin{array}{r l}{\quad}& {{}\operatorname*{max}_{a\in\mathcal{A}} Q_{\star}(s,a)=}\end{array}$ 。基于状态 $s$ ，智能体应该执行动作。 
![](https://cdn-mineru.openxlab.org.cn/model-mineru/prod/3a87ca1000e9fb65aa9881d7642678067e3f598fd33a7ae0a224ecdfc0d86ddb.jpg) 
17. 驾车按照路线“甲 $\rightarrow$ 乙 $\rightarrow$ 丙”行驶。从甲地出发时，模型预计需要行驶20 小时。 
实际行驶6 小时到达乙地。模型预计还需12 小时才能到达丙地。如果我们用TD算法更新模型，那么TD 目标是 $\widehat{y}=\underline{{{\qquad\qquad\qquad}}}$ 小时，TD 误差的绝对值是 $|\delta|\,=$ 小时。 
18. 同策略（on-policy）使用经验回放。异策略（oﬀ-policy）使用经验回放。 
A. 允许 B. 不允许 
19. Q 学习用控制智能体与环境交互。 
