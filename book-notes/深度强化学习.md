...
$$
 b 
33. 把 $\widehat{y}_{j}$ 看做常数，做一次梯度下降更新参数： 
   
$$
...
$$ 
39. 设 $\tau\in(0,1)$ 是需要手动调的超参数。做加权平均更新目标网络的参数： 
   
$$
...
$$ 
48. 设 $\tau\in(0,1)$ 是需要手动调整的超参数。做加权平均更新目标网络的参数： 
   
$$
\pmb{w}_{\mathrm{new}}^{-}\ \leftarrow\ \tau\cdot\pmb{w}_{\mathrm{new}}\,+\,\left(1-\tau\right)\cdot\pmb{w}_{\mathrm{new}}^{-}.
$$ 
...
49. 用优先经验回放，从数组中抽取一个四元组，记作 $(s_{j},a_{j},r_{j},s_{j+1})$ 。2. 用标准正态分布生成 $\xi$ 。对噪声DQN 做正向传播，得到： 
   
$$
...
$$ 
51. 用标准正态分布生成 $\xi^{\prime}$ 。用目标网络计算价值： 
   
$$
...
$$
 b b b b 
53. 设 $\alpha_{\mu}$ 和 $\alpha_{\sigma}$ 为学习率。做梯度下降更新噪声DQN 的参数： 
$$
...
$$
 e 
54. 设 $\tau\in(0,1)$ 是需要手动调整的超参数。做加权平均更新目标网络的参数： 
$$
...
# $\iff$ 第6 章习题k 
55. 设置经验回放数组的大小是 $10^{6}$ 。当数组中存满 $10^{6}$ 条四元组的时候，应该怎么办？ 
A. 删除数组中最旧的四元组。B. 删除数组中最新的四元组。C. 终止程序，停止强化学习。D. 继续运行程序，但是不再收集经验。 
56. 我们设置Q 学习的批大小(batch size) 为128，设置经验回放数组的大小是 $10^{6}$ 。初始的时候数组是空的。当数组中四元组的数量超过某个阈值 $k$ 的时候，开始做经验回放更新DQN。请问 $k$ 该如何设置？ 
A. 设置 $k$ 等于128 或略大于128。B. 设置 $k$ 远大于128 但是小于 $10^{6}$ 。C. 设置 $k$ 等于 $10^{6}$ 。 
57. 在优先经验回放中，需要对四元组 $\{(s,a,r,s^{\prime})\}$ 做非均匀抽样。请问抽样的概率取决于什么？ 
A. TD 目标的绝对值 $|\widehat{\boldsymbol{y}}|$ B. TD 目标的绝对值 $|\delta|$ | | 。 C. Q 值的绝对值 $|Q(s,a;w)|$ 。D. 奖励的绝对值 $|r|$ 。 
58. 在优先经验回放中，如果一个四元组被抽样的概率较大，那么它的学习率。 
A. 较大B. 较小 
59. DQN 的高估问题是由引起的。 
A. DQN 网络结构的缺陷 B. Q 学习算法的缺陷 C. A 和 B 都对 
60. 训练DQN 的时候使用目标网络(target network) 的好处在于缓解。 
A. 自举造成的偏差B. 最大化造成的高估C. A 和 B 都对 
61. 
62. 
63. 比起 79. 比起 Q 学习算法，双 Q 学习的优势在于缓解。成的偏差B. 最大化造成的高估C. A 和 B 都对 
64. 最优优势函数(optimal advantage function) 的定义是： 
         
$$
D_{\star}\bigl(s,a\bigr)\;\triangleq\;Q_{\star}\bigl(s,a\bigr)\;-\;V_{\star}\bigl(s\bigr).
$$ 
...
65. 为什么对决网络的表现优于简单的DQN？ 
A. 对决网络的神经网络结构更好。B. 训练对决网络的算法更好。C. A 和B 都对。 
66. Rainbow 方法〔48〕是对的改进。 
...
67. 用策略网络 $\scriptstyle\theta_{\mathrm{now}}$ 控制智能体从头开始玩一局游戏，得到一条轨迹(trajectory)： 
$$
...
$$ 
69. 用 $\{(s_{t},a_{t})\}_{t=1}^{n}$ 作为数据，做反向传播计算： 
    
$$
...
71. 观测到当前状态 $s_{t}$ ，根据策略网络做决策： $a_{t}\sim\pi(\,\cdot\,|\,s_{t};\,\theta_{\mathrm{now}})$ ，并让智能体执行动 
作 $a_{t}$ 。 
72. 从环境中观测到奖励 $r_{t}$ 和新的状态 $s_{t+1}$ 。 
73. 根据策略网络做决策： $\tilde{a}_{t+1}\sim\pi\big(\cdot|s_{t+1};\theta_{\mathrm{now}}\big)$ ，但不让智能体执行动作 $\tilde{a}_{t+1}$ 。 
74. 让价值网络打分： 
      
$$
...
78. 观测到当前状态 $s_{t}$ ，根据策略网络做决策： $a_{t}\sim\pi(\,\cdot\,|\,s_{t};\,\theta_{\mathrm{now}})$ ，并让智能体执行动作 $a_{t}$ 。 
79. 从环境中观测到奖励 $r_{t}$ 和新的状态 $s_{t+1}$ 。 
80. 根据策略网络做决策： $\tilde{a}_{t+1}\sim\pi\big(\cdot\,\big|\,s_{t+1};\,\theta_{\mathrm{now}}\big)$ ，但是不让智能体执行动作 $\tilde{a}_{t+1}$ 。 
81. 让价值网络给 $(s_{t},a_{t})$ 打分： 
   
$$
\widehat{q}_{t}\;=\;q\big(s_{t},a_{t};\,\pmb{w}_{\mathrm{now}}\big).
$$
 b 
82. 让目标网络给 $(s_{t+1},\tilde{a}_{t+1})$ 打分： 
   
$$
...
$$
 b 
86. 设 $\tau\in(0,1)$ 是需要手动调整的超参数。做加权平均更新目标网络的参数： 
   
$$
\pmb{w}_{\mathrm{new}}^{-}\ \leftarrow\ \tau\cdot\pmb{w}_{\mathrm{new}}\,+\,\left(1-\tau\right)\cdot\pmb{w}_{\mathrm{new}}^{-}.
$$ 
...
# $\iff$ 第7 章习题k 
87. 把策略网络记作 $\pi(a|s103. 把策略网络记作 $\pi(a|s;\pmb\theta)$ 。请计算： $\begin{array}{r}{\sum_{a\in\mathcal{A}}\left|\pi(a|s;\pmb\theta)\right|\,=\,\underline }\end{array}$ 。∈A }\end{array}$ 。∈A ax 做激活函数？ 
88. 状态价值函数 $V_{\pi}(s_{t})$ 依赖于未来的状态 $s_{t+1},s_{t+1},\cdot\cdot\cdot$ 。 
A. 上述说法正确。B. 上述说法错误。 
89. 设 $\ln x$ 为 $x$ 的自然对数。它的导数是 ${\frac{d\ln x}{d x}}={\frac{}{-\ \ \ \ \ \ \ }}$ 。 
A. $x^{2}$ B. $x$ C. $\textstyle{\frac{1}{x}}$ D. ln $x$ E. $e^{x}$ 
90. REINFORCE 的原理是对策略梯度 $\frac{\partial\,J(\theta)}{\partial\,\theta}$ 做蒙特卡洛近似。请你总结一下REIN-FORCE 做了哪些近似。 
91. 在actor-critic 的训练中使用目标网络，目的在于。 
...
92. 用策略网络 $\scriptstyle\theta_{\mathrm{now}}$ 控制智能体从头开始玩一局游戏，得到一条轨迹（trajectory）： 
$$
s_{1},a_{1},r_{1},\quad s_{2},a_{2},r_{2},\quad\cdots\cdot,\quad s_{n},a_{n},r_{n}.
$$ 
93. 计算所有的回报： 
...
$$ 
95. 计 $\delta_{t}=\widehat{v}_{t}-u_{t}$ ， $\forall\,t=1,\cdot\cdot\cdot\,,n\,_{\odot}$ ∀ · · ·。 
96. 用 $\{s_{t}\}_{t=1}^{n}$ {} 作为价值网络输入，做反向传播计算： 
   
$$
...
$$ 
98. 用 $\{(s_{t},a_{t})\}_{t=1}^{n}$ 作为数据，做反向传播计算： 
    
$$
\nabla_{\theta}\ln\pi\big(a_{t}\,\big|\,s_{t};\,\theta_{\mathrm{now}}\big),\qquad\forall\,t=1,\cdot\cdot\cdot\,,n.
$$ 
...
100. 观测到当前状态 $s_{t}$ ，根据策略网络做决策： $a_{t}\sim\pi(\,\cdot\,|\,s_{t};\,\theta_{\mathrm{now}})$ ，并让智能体执行动作 $a_{t}$ 。 
101. 从环境中观测到奖励 $r_{t}$ 和新的状态 $s_{t+1}$ 。 
102. 让价值网络打分： 
      
$$
...
106. 观测到当前状态 $s_{t}$ ，根据策略网络做决策： $a_{t}\sim\pi(\,\cdot\,|\,s_{t};\,\theta_{\mathrm{now}})$ ，并让智能体执行动作 $a_{t}$ 。 
107. 从环境中观测到奖励 $r_{t}$ 和新的状态 $s_{t+1}$ 。 
108. 让价值网络给 $s_{t}$ 打分： 
   
$$
...
\pmb{\theta}_{\mathrm{new}}\ \leftarrow\ \pmb{\theta}_{\mathrm{now}}\ -\ \beta\cdot\delta_{t}\cdot\nabla_{\pmb{\theta}}\ln\pi\big(a_{t}\,\big|\,s_{t};\,\pmb{\theta}_{\mathrm{now}}\big).
$$ 
113. 设 $\tau\in(0,1)$ 是需要手动调的超参数。做加权平均更新目标网络的参数： 
   
$$
\pmb{w}_{\mathrm{new}}^{-}\ \leftarrow\ \tau\cdot\pmb{w}_{\mathrm{new}}\,+\,\left(1-\tau\right)\cdot\pmb{w}_{\mathrm{new}}^{-}.
$$ 
...
# $\iff$ 第8 章习题k 
114. 是否可以用最优动作价值函数 $Q_{\star}(s,a)$ 作为基线？ 
A. 可以，因为 $Q_{\star}$ 不依赖于动作。B. 不行，因为 $Q_{\star}$ 依赖于动作。 
115. 带基线的REINFORCE 算法属于Actor-Critic。 
...
116. 做近似——构造函数 $\tilde{L}$ 近似目标函数 $J(\pmb\theta)$ ： 
...
123. 让目标策略网络 $\widehat{\pmb{a}}_{j+1}^{-}=\pmb{\mu}(s_{j+1};\pmb{\theta}_{\mathrm{now}}^{-})+\pmb{\xi}$ 。其中向量 $\xi$ 的每个元素都独立从截断正态分布 $\mathcal{C N}(0,\,\sigma^{2},-c,c)$ CN − 中抽取。 
124. 让两个目标价值网络做预测： 
      
$$
...
$$ 
129. 每隔 $k$ 轮更新一次策略网络和三个目标网络：让策略网络做预测： $\widehat{\mathbf{a}}_{j}=\pmb{\mu}(s_{j};\pmb{\theta})$ 。然后更新策略网络： 
      
$$
...
130. 观测到当前状态 $s$ ，预测均 ${\widehat{\mu}}=\mu(s)$ 标准差 ${\widehat{\sigma}}=\sigma(s)$ 。 
131. 从正态分布中做随机抽样： $\boldsymbol{a}\sim\mathcal{N}(\widehat{\mu},\widehat{\sigma}^{2})$  ；智能体执行动作 $a$ 。 
...
132. 当前状态 $s$ ，计算均值 $\widehat{\mu}=\mu(s;\pmb\theta)$ ，方差对数 ${\widehat{\rho}}=\rho(s;\pmb\theta)$ ，以及方差 $\widehat{\sigma}^{2}=$ $\exp({\widehat{\rho}})\,.$ 。 
133. 从正态分布中做随机抽样：b $a\sim\mathcal{N}(\widehat{\mu},\widehat{\sigma}^{2})$ ；智能体执行动作 $a$ 。 
...
134. 搭建均值网络 $\pmb{\mu}(s;\pmb{\theta})$ 、方差对数网络 $\rho(s;\pmb\theta)$ 、辅助网络 $f(s,\mathbf{}\mathbf{}\mathbf{}\mathbf{};\pmb{\theta})$ 。2. 让智能体与环境交互，记录每一步的状态、动作、奖励，并对参数 $\pmb{\theta}$ 做更新： 
...
135. 用 $\pmb{\mu}(s;\pmb{\theta}_{\mathrm{now}})$ 和 $\pmb{\rho}(s;\pmb{\theta}_{\mathrm{now}})$ 控制智能体与环境交互，完成一局游戏，得到一条轨迹： 
$$
...
139. 实际观测到当前状态 $s_{t}$ ，用控制器算出均值 $\pmb{\mu}(s_{t};\pmb{\theta}_{\mathrm{now}})$ 和方差对数 $\pmb{\rho}(s_{t};\pmb{\theta}_{\mathrm{now}})$ ，然后随机抽样得到动作 $\mathbf{\delta}a_{t}$ 。智能体执行动作 $\mathbf{a}_{t}$ ，观测到奖励 $r_{t}$ 与新的状态 $s_{t+1}$ 。 
140. 计算均值 $\pmb{\mu}(s_{t+1};\pmb{\theta}_{\mathrm{now}})$ 和方差对数 $\pmb{\rho}(s_{t+1};\pmb{\theta}_{\mathrm{now}})$ ，然后随机抽样得到动作 $\tilde{\mathbf{a}}_{t+1}$ 。这个动作只是假想动作，智能体不予执行。 
141. 用价值网络计算出： 
   
$$
...
147. 图11.6 中是一个循环神经网络。用这个公式更新 $h$ ： 
$$
\pmb{h}_{t}\ =\ \operatorname{tanh}\Big(\pmb{W}\left[h_{t-1};\,\pmb{x}_{t}\right]\,+\,\pmb{b}\Big).
$$ 
...
148. 从序号 $\{1,\cdot\cdot\cdot\ ,n\}$ 中做均匀随机抽样，把抽到的序号记作 $j$ 。 
149. 设当前策略网络参数为 $\theta_{\mathrm{now}}$ 。把 $s_{j}\backsim\mathbf{\nabla}a_{j}$ 作为输入，做反向传播计算梯度，然后用梯度更新 $\pmb{\theta}$ ： 
   
$$
...
150. 从序号 $\{1,\cdot\cdot\cdot\ ,n\}$ 中做均匀随机抽样，把抽到的序号记作 $j$ 。 
151. 设当前策略网络的参数是 $\scriptstyle\theta_{\mathrm{now}}$ 。把 $s_{j},\ \bar{\mathbf{a}}_{j}$ 作为输入，做反向传播计算梯度，然后用梯度更新 $\pmb{\theta}$ ： 
$$
...
152. 更新判别器 $D(x;\phi)$ 的参数：(a). 计算平均梯度： 
$$
...
153. 从训练数据集中均匀抽样一条轨迹，记作 
$$
\begin{array}{r l r}{\tau^{\mathrm{real}}}&{=}&{\left[s_{1}^{\mathrm{real}},\,a_{1}^{\mathrm{real}},\,\,\cdot\cdot\cdot\,,\,\,s_{m}^{\mathrm{real}},a_{m}^{\mathrm{real}}\right].}\end{array}
$$ 
154. 用策略网络 $\pi(a|s;\theta_{\mathrm{now}})$ 控制智能体与环境交互，得到一条轨迹，记作 
$$
...
$$ 
155. 用判别器评价策略网络的决策是否真实： 
$$
u_{t}\ =\ \ln D\Big(s_{t}^{\mathrm{fake}},\,a_{t}^{\mathrm{fake}};\ \phi_{\mathrm{now}}\Big),\qquad\forall\,t=1,\cdot\cdot\cdot\,,n.
$$ 
156. 把 $\tau^{\mathrm{fake}}$ 和 $u_{1},\cdot\cdot\cdot\ ,u_{n}$ 作为输入，用公式(12.2) 更新生成器（即策略网络）的参数，得到 $\pmb{\theta}_{\mathrm{new}}$ 。 
157. 把 $\tau^{\mathrm{real}}$ 和 $\tau^{\mathrm{fake}}$ 作为输入，用公式(12.3) 更新判别器参数，得到 $\phi_{\mathrm{new}}$ 。 
# $\backsimeq$ 第12 章知识点N 
...
158. 广播（broadcast）：服务器将当前的模型参数 ${\pmb w}_{\mathrm{n ow}}$ 广播到 $m$ 个worker 节点。这样一来，所有节点都知道 ${\pmb w}_{\mathrm{nlow}}$ 。 
159. 映射（map）：这一步让 $m$ 个worker 节点做并行计算，用本地数据计算梯度。需要 
...
160. 规约（reduce）：在做完映射之后，向量 $\boldsymbol{z}_{1},\cdots,\boldsymbol{z}_{n}\in\mathbb{R}^{d}$ 分布式存储在 $m$ 个worker节点上，每个节点有一个子集。不难看出，目标函数 $\begin{array}{r}{L(\pmb{w})=\frac{1}{2n}\sum_{j=1}^{n}(\pmb{x}_{j}^{T}\pmb{w}-y_{j})^{2}}\end{array}$ P在 ${\pmb w}_{\mathrm{now}}$ 处的梯度等于： 
$$
...
161. 更新参数：最后，服务器在本地做梯度下降，更新模型参数： 
   
$$
...
162. 钟表时间（wall-clock time），也叫elapsed real time，意思是程序实际运行的时间。可以这样理解钟表时间：在程序开始运行的时候，记录下墙上钟表的时刻；在程序结束的时候，再记录钟表的时刻；两者之差就是钟表时间。 
163. 处理器时间(CPU time 或GPU time) 是所有处理器运行时间的总和。比如使用4 块CPU 做并行计算，程序运行的钟表时间是1 分钟，期间CPU 没有空闲，那么系统的CPU 时间等于4 分钟。 
...
164. 向服务器发出请求，索要最新的模型参数。把接收到的参数记作 ${\pmb w}_{\mathrm{now}}$ 。 
165. 利用本地的数据 $\left\{(\pmb{x}_{j},y_{j})\right\}_{j\in\mathcal{Z}_{k}}$ 	 和参数 ${\pmb w}_{\mathrm{nlow}}$ 计算本地的梯度：∈I 
$$
\tilde{\pmb g}^{k}\ =\ \frac{1}{|\mathcal{Z}_{k}|}\sum_{j\in\mathcal{Z}_{k}}\big(\pmb x_{j}^{T}\pmb w_{\mathrm{now}}-y_{j}\big)\pmb x_{j}.
$$ 
166. 把计算出的梯度 $\tilde{g}^{k}$ 发送给服务器。 
...
167. 不难证明，同步并行梯度下降更新参数的方式为： 
   
$$
\pmb{w}_{\mathrm{new}}\ \leftarrow\ \pmb{w}_{\mathrm{new}}\ -\ \eta\cdot\nabla_{\pmb{w}}L\big(\pmb{w}_{\mathrm{new}}\big),
$$ 
即标准的梯度下降。在同一时刻，所有worker 节点上的参数是相同的，都是 ${\pmb w}_{\mathrm{nlow}}$ 。所有worker 节点都基于相同的 ${\pmb w}_{\mathrm{now}}$ 计算梯度。 
168. 对于异步并行梯度下降，在同一时刻，不同worker 节点上的参数 $\mathbfit{w}$ 通常是不同的。比如两个worker 分别在 $t_{1}$ 和 $t_{2}$ 时刻向服务器索要参数。在两个时刻之间，服务器可能已经对参数做了多次更新，导致在 $t_{1}$ 和 $t_{2}$ 时刻取回的参数不同。两个worker节点会基于不同的参数计算梯度。 
...
169. 向服务器发出请求，索要最新的DQN 参数。把接收到的参数记作 $\pmb{w}_{\mathrm{new}}$ 。 
170. 更新本地的目标网络： 
   
$$
\pmb{w}_{\mathrm{new}}^{-}\ \leftarrow\ \tau\cdot\pmb{w}_{\mathrm{new}}\,+\,\left(1-\tau\right)\cdot\pmb{w}_{\mathrm{new}}^{-}.
...
\tilde{\pmb{g}}^{k}\ =\ \nabla_{\pmb{w}}\,L\big(\pmb{w}_{\mathrm{new}}\big).
$$ 
172. 把计算出的梯度 $\tilde{\boldsymbol{g}}^{k}$ 发送给服务器。 
...
173. 向服务器发出请求，索要最新的参数。把接收到的参数记作 $\pmb{\theta}_{\mathrm{new}}$ 、 $\pmb{w}_{\mathrm{new}}$ 。 
174. 更新本地的目标网络： 
   
$$
\pmb{w}_{\mathrm{new}}^{-}\ \leftarrow\ \tau\cdot\pmb{w}_{\mathrm{new}}\,+\,\left(1-\tau\right)\cdot\pmb{w}_{\mathrm{new}}^{-}.
$$ 
175. 重复下面的步骤 $b$ 次（ $\dot{\mathbf{\eta}}^{b}$ 是用户设置的超参数），或是从头到尾完成一回合游戏。让智能体与环境交互，计算策略梯度，并累积策略梯度。全零初始化： $\tilde{g}_{\theta}^{k}\gets\mathbf{0}$ ←、 $\tilde{g}_{w}^{k}\gets\mathbf{0}$ ←，用它们累积梯度。 
...
$$ 
176. 把累积的梯度 $\tilde{\pmb g}_{\theta}^{k}$ 和 $\tilde{\pmb{g}}_{w}^{k}$ 发送给服务器。 
...
# $\iff$ 第13 章习题k 
177. 如果在1 个节点（compute node）上运行某算法，需要100 秒才能收敛。如果在8个节点上运行该算法，需要20 秒收敛。加速比（speedup ratio）等于。 
178. 使用更多的节点做并行计算，则通信所需的钟表时间会。 
A. 升高B. 降低 C. 不变 
179. 单机的梯度下降算法与同步并行梯度下降算法在数学上是等价的。 
A. 这种说法正确。B. 这种说法错误。 
180. 同步并行梯度下降算法与异步并行梯度下降算法在数学上是等价的。 
A. 这种说法正确。B. 这种说法错误。 
# u 第13 章相关文献U 
...
181. 观测到当前状态 $s_{t}=[o_{t}^{1},\cdot\cdot\cdot\,,o_{t}^{m}]$ · · ·，让每一个智能体独立做随机抽样： 
    
$$
a_{t}^{i}\ \sim\ \pi\big(\cdot\ \big|\ s_{t};\,\theta_{\mathrm{now}}^{i}\big),\qquad\forall\;i=1,\cdot\cdot\cdot\ ,m,
$$ 
并执行选中的动作。 
182. 从环境中观测到奖励 $r_{t}$ 与下一时刻状态 $s_{t+1}=\left[o_{t+1}^{1},\cdot\cdot\cdot\,,o_{t+1}^{m}\right]\!.$ ' ' 。 
183. 让价值网络做预测： $\widehat{v}_{t}=v\big(s_{t};\pmb{w}_{\mathrm{now}}\big)$  。4. 让目标网络做预测： $\widehat{v}_{t+1}^{-}=v\bigl(s_{t+1};w_{\mathrm{now}}^{-}\bigr)\,.$ b 。 
184. 计算TD 目标与TD 误差： 
$$
...
188. 在 $t$ 时刻，智能体 $i$ 观测到 $o_{t}^{i}$ ，然后做随机抽样 $a_{t}^{i}\sim\pi(\,\cdot\,|\,o_{t}^{i};\,\pmb\theta^{i})$ ∼ · |，并执行选中的动作 $a_{t}^{i}$ 。 
189. 环境反馈给智能体奖励 $r_{t}$ 与新的观测 $o_{t+1}^{i}$ 。3. 让价值网络做预测： $\widehat{v}_{t}^{i}=v\big(o_{t}^{i};\pmb{w}_{\mathrm{now}}^{i}\big)$  。4. 让目标网络做预测： $\begin{array}{r}{\widehat{v}_{t+1}^{i}=v\big(o_{t+1}^{i};\pmb{w}_{\mathrm{now}}^{i-}\big)\,.}\end{array}$ b 。 
190. 计算TD 目标与TD 误差： 
$$
...
194. 每个智能体 $i$ 与环境交互，获取当前观测 $o_{t}^{i}$ ，独立做随机抽样： 
    
$$
a_{t}^{i}\ \sim\ \pi\big(\cdot\ \big|\ o_{t}^{i};\,\theta_{\mathrm{now}}^{i}\big),\qquad\forall\;i=1,\cdot\cdot\cdot\,,m,
$$ 
并执行选中的动作。 
195. 下一时刻，每个智能体 $i$ 都观测到 $o_{t+1}^{i}$ 。假设中央控制器可以从环境获取奖励 $r_{t}$ ，或者向智能体询问奖励 $r_{t}$ 。 
...
196. 每个智能体 $i$ 向中央控制器传输观测 $o_{t}^{i}$ 和 $o_{t+1}^{i}$ ；中央控制器得到状态 
' ' ' ' 
$$
...
$$ 
197. 中央控制器让价值网络做预测： $\widehat{v}_{t}\;=\;v\big(s_{t};{\pmb w}_{\mathrm{now}}\big)$  。 
198. 中央控制器让目标网络做预测： $\widehat{v_{t+1}}\ =\ v\left(s_{t+1};\mathbf{w}_{\mathrm{now}}^{-}\right).$ b 。 
199. 中央控制器计算TD 目标和TD 误差： 
$$
\widehat{y_{t}}\ =\ r_{t}+\gamma\cdot\widehat{v}_{t+1}^{-},\qquad\delta_{t}\ =\ \widehat{v}_{t}-\widehat{y}_{t}^{-},
$$
 b b b b 
...
# $\iff$ 第15 章习题k 
203. 设动作 $A=[A^{1},\cdot\cdot\cdot,A^{m}]$ 的概率质量函数为 
            
$$
\pi\big(A\,\big|\,S;\pmb\theta^{1},\cdot\cdot\cdot\,,\pmb\theta^{m}\big)\ \ \triangleq\ \ \pi\big(A^{1}\,\big|\,S;\pmb\theta^{1}\big)\ \times\ \cdot\cdot\cdot\ \times\ \pi\big(A^{m}\,\big|\,S;\pmb\theta^{m}\big).
$$ 
...
204. 观测到当前状态 $s_{t}=[o_{t}^{1},\cdot\cdot\cdot\ ,o_{t}^{m}]$ · · ·，让每一个智能体独立做随机抽样： 
    
$$
a_{t}^{i}\ \sim\ \pi\big(\cdot\ \big|\ s_{t};\,\theta_{\mathrm{now}}^{i}\big),\qquad\forall\,i=1,\cdot\cdot\cdot\ ,m,
$$ 
并执行选中的动作。 
205. 从环境中观测到奖励 $r_{t}^{1},\cdot\cdot\cdot\mathbf{\Omega},r_{t}^{m}$ · · ·与下一时刻状态 $s_{t+1}=\left[o_{t+1}^{1},\cdot\cdot\cdot\,,o_{t+1}^{m}\right]$ ''。 
206. 让价值网络做预测： 
   
$$
...
212. 每个智能体 $i$ 与环境交互，获取当前观测 $o_{t}^{i}$ ，独立做随机抽样： 
    
$$
a_{t}^{i}\ \sim\ \pi\big(\cdot\ \big|\ o_{t}^{i};\,\theta_{\mathrm{now}}^{i}\big),\qquad\forall\;i=1,\cdot\cdot\cdot\,,m,
$$ 
(16.1) 
并执行选中的动作。 
213. 下一时刻，每个智能体 $i$ 都观测到 $o_{t+1}^{i}$ 和收到奖励 $r_{t}^{i}$ 。 
214. 每个智能体 $i$ 向中央控制器传输观测 $o_{t}^{i}$ 、 $o_{t+1}^{i}$ 、 $r_{t}^{i}$ ；中央控制器得到状态 
' ' ' ' 
$$
s_{t}\ =\ \left[o_{t}^{1},\cdot\cdot\cdot\ ,o_{t}^{m}\right]\qquad\sharp\mathbb{I}\qquad s_{t+1}\ =\ \left[o_{t+1}^{1},\cdot\cdot\cdot\ ,o_{t+1}^{m}\right].
$$ 
...
$$
 b 
228. 更新所有 $2m$ 个目标网络： $\forall\:i=1,\cdot\cdot\cdot\;,m$ ， 
$$
...
229. 如图17.2 所示，对于所有的 $i=1,\cdot\cdot\cdot\;,m$ ，把输入的 $\mathbf{\nabla}x^{i}$ 映射到三元组 $(q^{i},k^{i},v^{i})$ ： 
$$
...
$$ 
230. 如图17.3 所示，计算权重向量 $(\alpha^{1},\cdot\cdot\cdot,\alpha^{m})$ ，每个权重向量的大小都是 $m\times1$ 。第 $i$ 个权重向量 $\alpha^{i}$ 依赖于 $q^{i}$ 和 $(\pmb{k}^{1},\cdot\cdot\cdot\mathbf{\Lambda},\pmb{k}^{m})$ ： 
$$
...
231. 如图17.4 所示，计算输出向量 $(c^{1},\cdot\cdot\cdot\mathbf{\Lambda},c^{m})$ ，每个输出向量的维度都是 $d_{\mathrm{out}}$ 。第 $i$ 个输出向量 $c^{i}$ 依赖于 $\alpha^{i}$ 和 $(\pmb{v}^{1},\cdot\cdot\cdot\mathbf{\sigma},\pmb{v}^{m})$ ： 
$$
...
232. 让MCTS 自我博弈，完成一局游戏，收集到 $n$ 个三元组 $(s_{1},p_{1},u_{1}),\cdot\cdot\cdot,(s_{n},p_{n},u_{n})\,\mathrm{.}$ 。2. 按照公式(18.2) 做一次梯度下降，更新策略网络参数 $\pmb{\theta}$ 。3. 按照公式(18.3) 做一次梯度下降，更新价值网络参数 $\mathbfit{w}$ 。 
# u 第18 章相关文献U 
...
# $\iff$ 第18 章习题k 
233. AlphaGo 用MCTS 来判断动作的好坏。通过计算，MCTS 发现动作 $a$ 的分数 $N(a)$ 很高，这说明。 
A. 动作 $a$ 好B. 动作 $a$ 不好 
234. AlphaGo 中的价值网络 $v(s;w)$ 是对的近似。 
...
235. 从搜索空间中均匀随机选出一组超参数的组合，搭建卷积神经网络。2. 在训练集上训练神经网络，从随机初始化开始，一直到梯度算法收敛。3. 在验证集评价神经网络，记录下验证准确率。 
