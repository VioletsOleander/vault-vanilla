...
>  在游戏尚未结束的 $t$ 时刻，折扣回报 $U_t$ 是一个随机变量，其随机性来自于 $t$ 时刻后未观测到的奖励 (进而随机性来自于 $t$ 时刻后未观测到的所有状态和动作)
...
>  根据最优动作价值函数，可以在给定状态下，为不同的动作打分，查看哪个动作可以最大化回报 $U_t$ 的期望
...
>  $\epsilon$ -greedy 策略的定义利用到了 DQN 本身
...
1. 把表格 ${\widetilde Q}_{\mathrm{now}}$ 中第 $({s}_{j},{a}_{j})$ 位置上的元素记作： 
...
>  可以参照 [[paper-notes/gen-ai/image/Spectral Normalization for Generative Adversarial Networks-2018-ICLR|Spectral Normalization for Generative Adversarial Networks-2018-ICLR]]
