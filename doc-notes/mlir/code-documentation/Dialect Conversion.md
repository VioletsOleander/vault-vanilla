...
>  该框架允许通过一组 pattern-based 的操作重写模式，将非法操作转化为由提供的转换目标支持的操作
...
>  部分转换
>  部分转换为目标合法化尽可能多的操作，但不会转换没有显式被标记为 “非法” 的操作，部分转换允许在输入 operations 中存在 “未知”  操作时仅 lowering 部分 operations
...
>  完全转换
>  完全转换会合法化所有输入操作，并且也仅在所有操作都被正确针对给定转换目标合法化后成功
...
>  分析转换
>  分析转换会分析哪些操作针对给定转换是可以合法化的。这实际上是通过先执行一个部分转换，然后记录哪些操作会被成功转换来实现的，在这个过程中，输入操作不会被实际重写或转换。
>  分析转换通过 `applyAnalysisConversion`
...
>  由转换框架生成的最终操作只有在 `ConverTarget` 中被标记是合法的，转换才会成功
...
>  操作和方言可以通过这些 legality actions 进行标记
>  - Legal: 该 action 表示给定操作的每个实例都是合法的，即任何属性、操作数、类型等组合都是有效的
>  - Dynamic: 该 action 表示给定操作的某些实例是合法的。这允许定义精细的约束条件，例如 `arith.addi` 只有在对 32 bit 整数进行操作时才是合法的
>  - Illegal: 该 action 表示给定操作没有实例是合法的。被标记为 Illegal 的操作必须被转换，否则转换不会成功。该 action 还允许在其他合法的方言中选择性地将特定操作标记为非法
...
>  即未明确被标记为 legal 也未明确被标记为 illegal 的操作和方言属于上述分类之外的 “未知” 操作，其处理方式有所不同
...
>  如果一个操作被标记为 recursively legal ，则其中嵌套的所有操作被会被认为合法，即便它们原本可能会被认为是 Illegal
...
>  定义好转换目标后，需要提供一组合法化模式用于将非法操作转化为合法操作
>  所提供的模式不需要生成直接对于目标合法的操作，框架会自动构建一个转换图，将非法操作转换为一组合法操作
...
>  例如，我们定义了支持操作 `foo.add` 的目标，当我们提供了以下模式：\[ `bar.add` -> `barz.add`, `barz.add` -> `foo.add` \] 后，框架会自动检测到该模式可以用于合法化 `bar.add` -> `foo.add` ，即便不是直接的转换，因此我们不需要为 `bar.add` -> `foo.add` 定义直接的合法化模式
...
>  例如，转换过程并不总是就地更新操作，而是创建一个事件映射 (事件例如替代和删除)，并只在整个转换过程成功时才应用这些更改
...
>  某些 patterns 依赖于使用操作的更新/重新映射的 operands，例如当操作定义的结果类型发生更改时
>  此时，通用的 `RewritePattern` 将不适用，因为被匹配的操作的 operands 类型将和用户期望的类型不对应 (用户期望的类型即用户编写的 patterns 中所期望的 operations 的 operands 的类型，这些类型可能不是原来的类型，需要先进行一次转换)
...
>  `ConversionPattern` 通过向 `matchAndRewrite` 方法提供一个额外参数：操作在转换之后应该使用的 operands 列表。
>  如果某个 operand 是一个未转换操作的结果，例如它已经是 legal 的，则使用原始 operand，这意味着列表中提供的 operands 和操作上的 operands 需要有 1 对 1 的非空对应关系
...
>  为转换模式提供的 remapped operands 的类型必须是该转换模式所期望的类型
...
>  如果 remapped operands 的类型不是 type converter 所期望的类型，并且无法执行到所期望类型的 materialization 操作，则在调用 `mathAndRewirte` hook 之前，pattern 的应用将失败
...
>  当一个 value 定义 (无论是块参数还是操作结果) 的类型正被改变时，该定义的使用者必须在转换过程中被更新
